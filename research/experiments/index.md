...
| 017 | 2026-08-10 | CPU Lane (host-cache expert compute) | ❌ Dead end (ที่พิสูจน์แล้ว) | **CPU lane ไม่ช่วย — CPU bandwidth-bound อยู่แล้ว**: ตอน experts ทั้งหมดอยู่ CPU, CPU ใช้แค่ 39–51% (core ว่าง แต่ DDR4 bandwidth อิ่ม) → ย้ายงานมา CPU มากขึ้น = แย่ลง. exp_cpu 13–16.4 tps vs exp_gpu 56.8–58.6 tps (VRAM 2.8 vs 10.9 GB). ฟิสิกส์เดียวกับ EXP-012 (disk→RAM) — นี่คือ RAM→CPU bandwidth wall — placement software แก้ไม่ได้บนเครื่องคงที่. **ปิด #2 — เหลือ lever เดียวใน Phase 4: #3 IQ2_XXS** |
| 019 | 2026-08-10 | Gemma 4 26B-A4B QAT+MTP (14.25 GB) — ท้าชิง Qwen3.6 | ✅ Complete | **QAT ชนะ: Thai gate 9/9 + Thai tonal 6/6 PERFECT** (คำแรกบนเครื่องนี้ที่ผ่านครบที่ความเร็วใช้ได้) — ข้าว/เข้า=โท, ข่าว/ไข่/ไก่=เอก, ไหม=จัตวา พร้อมตัวอย่างประโยค. **MTP +20%** (37.6→45.1 warm) — ต่างจาก DS V4 MTP (EXP-015 −11–18%) เพราะ draft แยกเล็ก 0.46 GB ถูก. สรุป: **Gemma 4 QAT+MTP = daily driver ใหม่สำหรับไทย** (45–47 tok/s ≈ IQ2_M แต่โมเดลใหม่กว่า + คุณภาพ QAT) / Qwen IQ2_XXS ยังเป็น non-Thai speed king (61–66) |
| 020 | 2026-08-11 | Gemma 4 QAT+MTP — config sweep (threads/fa/KV) | ✅ Complete | **t12 = optimum: warm 48.99 (+12.7% เทียบ t8) + gate sustained 50.71 tok/s** — ไทย 9/9 เหมือนเดิม (thai_tonal reasoning ถูกใน think; final ถูกตัดเพราะ budget 4096 = artifact ไม่ใช่ regression). t16 ถอย (44.8), fa off ≈ on, KV q8 ≈ f16 ที่ ctx 2048 → **config ใหม่: `-t 12 -fa on` + MTP draft = ~49-51 tok/s** (จาก EXP-019 t8 45) — กำไร +13% จาก flag เดียว, คุณภาพคงเดิม |
| 022 | 2026-08-11 | Gemma 4 12B QAT+MTP — ตัวท้าชิงเล็ก | ✅ Complete | **BREAKTHROUGH: Thai-safe เร็วสุดบนเครื่องนี้ — 75.7 tok/s sustained + Thai gate 9/9 + tonal 6/6** (ข้าว/เข้า=โท, ข่าว/ไข่/ไก่=เอก, ไหม=จัตวา) — 6.72 GB พอดี VRAM 12 GB (faults/tok ~450 vs 1200 ของ 26B, ไม่ spill, threads ไม่มีผล t8≈t12). **ชนะ 26B +52%** และเหนือ Qwen speed kings (IQ1_M 74.7 / IQ2_XXS 61–66) ที่ไทย FAIL — ตอนนี้เร็ว+ไทย อยู่ด้วยกันได้แล้วบน 12 GB. ข้อแลกซื่อตรง: 26B เก่งกว่าโดยรวม (โมเดลใหญ่กว่า) → 12B = daily driver เร็ว, 26B = quality-first |
| 021 | 2026-08-11 | Engine swap — mainline b10357 vs Jan b9967 (proxy ของ ik) | ✅ Complete | **b10357 ≈ b9967 (cold 62.3 / warm 56.7 vs 43–62 / 52–61) — engine ไม่ใช่คอขวด** → ik_llama.cpp build ถูกเลื่อน (เห็น IK_LLAMA_EVAL.md): upstream mainline ไม่ชนะ + 110 tok/s ของ community วัดบน 4070S ไม่ใช่ 3060. **บทเรียน: prebuilt ของ official ต้องโหลด `cudart-llama-bin` แยก** — ไม่งั้น CUDA fallback เป็น CPU เงียบๆ (รอบแรกได้ 9 tok/s แล้วทิ้ง) — ตรวจ `--list-devices` ทุกครั้งที่สลับ engine |
| 018 | 2026-08-10 | IQ2_XXS — lever สุดท้าย Phase 4, วัดด้วย harness ตัวใหม่ | ✅ Complete | **Hypothesis ถูก reject: 0.7 GB ที่เพิ่มจาก IQ1_M ไม่คืน Thai tonal** — IQ2_XXS (10.02 GB) ได้ **61–66 tok/s แต่ Thai tonal 1/6 ถูก** (ข้าว/เข้า=เอก ❌ ควรโท, ไข่/ไก่=ตรี ❌ ควรเอก, ไหม=ตรี ❌ ควรจัตวา; ข่าว ถูก) — confident-but-wrong เหมือน IQ1_M. คุณภาพ non-Thai 8/9 เท่ากันทั้งสาม quant. **Phase 4 ปิดครบแล้ว**: เพดานซื่อตรง = IQ2_M 43–56 (ไทย) / IQ2_XXS 61–66 (non-Thai) / IQ1_M 74.7 (speed-first). + Harness ตัวแรกใช้งานจริง: `weight_stream/bench/` + `python -m weight_stream bench` (clean room, cmdline verify, cold/warm, Thai gate) — เจอ+แก้ 2 gap (JSON export ทิ้ง gate, thai_tonal ต้อง >2048 tok) + `scripts/resume_download.py` (Range-append + byte-exact หลัง download ค้าง 97.6%) |
| 023 | 2026-08-11 | Re-verify Thai gate หลัง config ใหม่ (t12 + MTP) | ✅ Complete | 12B repetition loop เกิดเฉพาะ bench path (temp-0) — product chat (temp 0.7 + max_tokens 1024) ไม่ loop; penalty matrix (repeat/presence/DRY) หนี attractor ไม่ได้ → bound output budget แทน (per-tier max_tokens) |
| 024 | 2026-08-12 | L1 prefetch A/B (E1/E2/E2b) | ❌ Negative result | Prefetch ล่วงหน้าไม่ชนะ page cache บน workload จริง — E1/E2/E2b ชุดทดสอบล้วน negative → ยืนยันว่า prefetch เป็น latency-hiding ไม่ใช่ throughput |
| 025 | 2026-08-12 | Simulator calibration — physics (TASKS #96/#150) | ✅ Complete | วัด BW จริง 3 tiers: cpu-ram **19.18** / gpu-vram **61.09** / disk-mmap **0.38 GB/s** (disk ต่ำกว่า NVMe spec 14 GB/s **37×** — page-fault read = random access) → `simulator/physics.py` + `calibrate.py` + 8 hermetic tests |
| 026 | 2026-08-12 | Streaming buffer abstraction (TASKS #145) | ✅ Complete | `BufferBackend` protocol: `SimulatorBufferAdapter` + `TelemetryBufferObserver` (แปลง OS signals เป็น buffer stats) → `BufferStatsView` เดียว — warm spike 22.02 vs measured 21.88 tok/s (Δ+0.6%) + 9 hermetic tests |
| 027 | 2026-08-12 | Phase 3b: real compute vs I/O ratio (TASKS #151) | ✅ Complete | Qwen1.5-MoE-A2.7B พอดี RAM = **compute-bound** (815 ms/token ceiling); 5% miss = 2.1s stall → buffer ไม่ช่วยเมื่อพอดี RAM |
| 028 | 2026-08-12 | Phase 4 evaluation — metrics + real benchmark (TASKS #159-162) | ✅ Complete | `weight_stream/eval/metrics.py` (hit rate / latency percentiles / throughput error) + `scripts/bench_phase4.py` — Qwen warm **22.73 tok/s vs physics 22.73 (+0.02%)**; p50 41.1 / p90 48.6 / p99 84.4 ms; hit rate 1.000 warm |
| 029 | 2026-08-12 | K3 (>RAM) benchmark with Phase 4 metrics | ✅ Complete | K3 15.6 GB/token: buffer 256MB → hit 51.2% → **0.049 tok/s**; buffer 4GB → hit 99.9% → **1.18 tok/s** (compute ceiling) = **24× upside** — buffer คุ้มเฉพาะ >RAM |
| 030 | 2026-08-13 | Expert offloading on fits-VRAM model | ✅ Complete | Qwen พอดี VRAM 12GB: `n-cpu-moe 0` = **126.6** vs `10` = **31.8 tok/s** (ช้าลง 4× — experts stream ข้าม PCIe) — offload คุ้มเฉพาะ >VRAM; baseline auto ≠ all-GPU → ต้องระบุ `--n-cpu-moe 0` เสมอ + 4 hermetic tests |
